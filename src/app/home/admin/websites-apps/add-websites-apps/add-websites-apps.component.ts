...
@Component({
  selector: 'add-websites-apps',
  templateUrl: './add-websites-apps.component.html',
  styleUrls: ['./add-websites-apps.component.scss'],
})
export class AddWebsiteAppComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AddWebsiteAppComponent>,
  ...
  ngDestroy(): void {
    this.submitAddWebsite.emit({ data: 'Dữ liệu trả về' });
  }
  /** Hàm khởi tạo form
   *
   */
              ...
              (res: any) => {
                if (res.vcode === 0) {
                  this.functionService.createMessage(
                    'success',
                    this.languageService.translate('them_website_app_thanh_cong')
                  );
  ...
  selectChange(event) {
    this.addWebsiteForm.controls['id_categorys'].setValue(event);
  }
  ...
  scrollToEditor(editor: any, main: any) {
    ...
    if (!data) {
      ...
      if (!range) {
        ...
        targetRect = rects.length > 0 ? rects[0] : editorRect;
      }
    }

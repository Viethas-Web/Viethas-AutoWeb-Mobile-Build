...
import { InfoContactDetailComponent } from './info-contact-detail/info-contact-detail.component';
...
export class InfoContactComponent implements OnInit {
  ...
  /**
   * Hàm cập nhật field isReplied cho contact
   * @param contactId 
   * @param isReplied 
   */
  updateContactIsReplied(contactId: string, isReplied: boolean) {
    this.vhQueryAutoWeb.updateContact(contactId, { isReplied: isReplied })
      .then((res: any) => {
        if (res.vcode === 0) {
          console.log(`Contact ${contactId} đã được cập nhật isReplied = ${isReplied}`);
        }
      })
      .catch((error) => {
        console.log('Lỗi khi cập nhật isReplied:', error);
      });
  }

  /**
   * Toggle trạng thái phản hồi của contact
   * @param e 
   * @param contact 
   */
  toggleReplyStatus(e: any, contact: any) {
    e.stopPropagation();
    
    const newStatus = !contact?.isReplied;
    const confirmTitle = newStatus 
      ? this.languageService.translate('ban_co_muon_danh_dau_la_da_phan_hoi')
      : this.languageService.translate('ban_co_muon_danh_dau_la_chua_phan_hoi');
    
    this.nzModalService.confirm({
      nzTitle: confirmTitle,
      nzContent: `<b>${contact.name}</b>`,
      nzOkText: this.languageService.translate('dong_y'),
      nzCancelText: this.languageService.translate('thoat'),
      nzWidth: 500,
      nzOnOk: () => {
        return new Promise((resolve, reject) => {
          this.vhQueryAutoWeb.updateContact(contact._id, { isReplied: newStatus })
            .then((res: any) => {
              if (res.vcode === 0) {
                contact.isReplied = newStatus;
                this.functionService.createMessage('success', 
                  newStatus ? 'da_danh_dau_phan_hoi' : 'da_huy_danh_dau_phan_hoi'
                );
                resolve(true);
              } else {
            ...
            .catch((error) => {
              console.log(error);
              this.functionService.createMessage('error', 'da_xay_ra_loi_vui_long_thu_lai');
              reject();
            });
        });
      }
  ...
  openDetail(item) {
     this.nzModalService.create({
      nzContent: InfoContactDetailComponent,
      nzFooter: null,
      nzWidth: '60vw',
      nzClosable: false,

...
@Component({
  selector: 'app-list-scripts-object',
  templateUrl: './list-scripts-object.component.html',
  styleUrls: ['./list-scripts-object.component.scss']
})
export class ListScriptsObjectComponent implements OnInit {
/** biến dùng để chứa chiều cao của bảng dữ liệu */
  ...
  handleAddData() {
    const modal: NzModalRef = this.nzModalService.create({
      ...
      nzCloseIcon: null,
      nzComponentParams: {
        type_setup: 'embedded_script_object',
        type_options: [
          { value: 'page', label: 'trang' },
          { value: 'block', label: 'khoi' },
          { value: 'object', label: 'vat_the' },
        ]
      }
    });

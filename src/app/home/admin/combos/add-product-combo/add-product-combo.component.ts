...
@Component({
  selector: 'app-add-product-combo',
  templateUrl: './add-product-combo.component.html',
  styleUrls: ['./add-product-combo.component.scss']
})
export class AddProductComboComponent implements OnInit {
  categoryInProducts: any
  comboIngredients: any
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AddProductComboComponent>,
  ...
  ngOnInit() {
    this.categoryInProducts = this.data.categoryInProducts
    this.comboIngredients = this.data.comboIngredients
  }
      ...
      .then((res: any) => {
        if (res.vcode === 0) {
          ...
          this.data_searched.forEach((product) => {
            product.isChoose = this.comboIngredients.some(prod => prod._id === product._id);
          })
 ...
 */
   ...
   */
   getProducts(sort) {
    ...
    this.loading = true;
    let queryFood = this.id_category == 'all' ? {} : { id_categorys: { $all: [this.id_category] } };
    let queryProduct = this.id_category == 'all' ? {} : { id_categorys: { $all: [this.id_category] } };

    let promise = this.currentValue == 'product' ?
 ...
 * @param productChoice
 */
 chooseProductToCombo(event: any, productChoice: any) {
  // const productExists: boolean = this.comboIngredientsTrash.some((product: any) => product._id === productChoice._id);
  // if (event === true) {
  //   // Dữ liệu được tryền vào combo
  //   if (!productExists) {
  //     this.comboIngredientsTrash.push({
  //       ...productChoice,
  //       quantity: 1,
  //       price: productChoice.units.find((item) => item.default == true).price,
  //       unit: productChoice.units.find((item) => item.default == true).unit,
  //       ratio: productChoice.units.find((item) => item.default == true).ratio,
  //     });
  //   }
  // } else {
  //   if (productExists) {
  //     this.comboIngredientsTrash = this.comboIngredientsTrash.filter(
  //       (filter) => filter._id !== productChoice._id
  //     );
  //   }
  // }
  if (event) {
    const defaultUnit = productChoice.units.find((item) => item.default === true);
  
    if (defaultUnit) {
      const unitFields = Object.keys(defaultUnit)
        .filter((key) => key.startsWith('unit_'))
        .reduce((acc, key) => {
          acc[key] = defaultUnit[key];
          return acc;
        }, {});
  
      this.comboIngredients.push({
        ...productChoice,
        quantity: 1,
        price: defaultUnit.price,
        ratio: defaultUnit.ratio,
        ...unitFields, // Gắn tất cả các trường bắt đầu bằng "unit_" từ item.default
      });
    }
  } else {
    this.comboIngredients = this.comboIngredients.filter((filter) => filter._id !== productChoice._id);
  }
}
  ...
  onAddProductToCombo() {
    this.dialogRef.close(this.comboIngredients);
  }

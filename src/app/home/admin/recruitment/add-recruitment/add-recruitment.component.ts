...
export class AddRecruitmentComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AddRecruitmentComponent>,
  ...
  initForm() {
    this.addRecruitmentForm = new FormGroup({
      date_expired: new FormControl(new Date()),
    ...
    this.newFields.filter((f: any) => f.field_input_type == "select-multiple").forEach((field: any) => {
      this.addRecruitmentForm.addControl(field.field_custom, new FormControl(field.field_start_value || [], field.field_required ? [Validators.required] : []));
    });


    this.functionService.multi_languages.forEach((language: any) => {
      this.functionService.adminAddHandleChangeMultiLanguage(
        this.addRecruitmentForm,
        language.code,
  ...
  saveRecruitment() {
    this.vhQueryAutoWeb.addRecruitment({
      ...this.addRecruitmentForm.value,
      link: this.functionService.nonAccentVietnamese(this.addRecruitmentForm.value['position_' + this.functionService.defaultLanguage].trim()).replace(/[^a-z0-9-]/g, '')
    }).then((res: any) => {
      console.log(res);
      if (res.vcode != 0) this.vhComponent.alertMessageDesktop("error", this.languageService.translate('tao_tin_tuyen_dung_khong_thanh_cong'));
      this.vhComponent.alertMessageDesktop("success", this.languageService.translate('tao_tin_tuyen_dung_thanh_cong'));
      this.dialogRef.close(res.data);
    });
  }

  getFormControl(controlName: string): FormControl {
    return this.addRecruitmentForm.get(controlName) as FormControl;
  }


  selectChange(event, field) {
    this.addRecruitmentForm.controls[field].setValue(event);
  }
  ...
  openCKEditorByDevice(): void {
    ...
    const dialogRef = this.matdialog.open(DescriptionByDeviceComponent, {
      ...
      disableClose: true,
      data: {
        formData: this.addRecruitmentForm,
  ...
  handleEditCkeditor(item) {
    const dialogRef = this.matdialog.open(VhCkeditorModalComponent, {
      ...
      data: {
        formData: this.addRecruitmentForm,
        item: item,
        type: 'recruitments' // để phân biệt upload ảnh cho danh mục hay sản phẩm
      }
    });
  ...
  public onReady(editor: any) {
    ...
    editor.plugins.get('FileRepository').createUploadAdapter = (loader: any) =>
      this.vhImage.MyUploadImageAdapter(loader, `images/database/recruitments`);
  }

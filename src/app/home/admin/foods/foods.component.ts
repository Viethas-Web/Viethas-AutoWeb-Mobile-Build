import { ChangeDetectorRef, Component, OnInit } from '@angular/core';
import { MatDialog } from '@angular/material/dialog';
import { VhAlgorithm, VhEventMediator, VhQueryAutoWeb } from 'vhautowebdb';
import { VhComponent } from 'src/app/components/vh-component/vh-component';
import { FunctionService } from 'vhobjects-service';
import { AddFoodComponent } from './add-food/add-food.component';
import { EditFoodComponent } from './edit-food/edit-food.component';
import { LanguageService } from 'src/app/services/language.service';

@Component({
  selector: 'app-foods',
  templateUrl: './foods.component.html',
  styleUrls: ['./foods.component.scss'],
})
export class FoodsComponent implements OnInit {
  /**Dùng tìm kiếm sản phẩm */
  keySearch: string = ''; 
  /** biến dùng để chứa chiều cao của bảng dữ liệu */
  tableHeight: string; 
  /** get những sp theo danh mục */
  idCategory = 'all';
  /** setup food */
  setupFoodImg
  /** setup category */
  setupCategoryImg
  /** danh sách danh mục để lọc */
  categories: Array<any> = [];
  /** mảng chưa danh sách sp search */  
  dataSearched: any  = [] 
  /** Danh sách sản phẩm hiển thị */
  foods: any = []; // 
  /** biến ẩn hiện loading ở table của sp khi get dữ liệu */
  loading = false;  
  loadingCategory = false;

  /** Mảng chứa số lượng sản phẩm hiển thị trong 1 trang */
  numberPerPage = [5, 10, 20, 50, 100]; 
  /* Biến này truyền vào hàm để sort */
  sort: any = { [this.nameField]: 1 };
  /* Biến này hiển thị ở html xem sort theo trường nào */
  sortby: any = { 
    [this.nameField]: true,
  };

  /** ------------------------Pagination------------------------ */
  /** Trang hiện tại */
  pageCurrent: number = 1;
  /** Tổng số trang */
  totalPages: number = 1;
  /** Số lượng bản ghi trên 1 trang */
  limit: number = 20;
  /** Số trang hiển thị = */
  pageShowChoose: any = [0, 1, 2]; 
  /** Trang đi đến  */
  pageGoto: number = 1;

  /** Lấy trường name theo ngôn ngữ đang được chọn */
  get nameField() {
    return 'name_' + this.functionService.selectedLanguageCode;
  }

  
  constructor(
    private vhComponent: VhComponent,
    public vhAlgorithm: VhAlgorithm,
    private vhQueryAutoWeb: VhQueryAutoWeb,
    public functionService: FunctionService,
    private changeDetectorRef: ChangeDetectorRef,
    private vhEventMediator: VhEventMediator,
    private dialog: MatDialog,
    private languageService: LanguageService
  ) { }

  ngOnInit(): void {
    this.getCategory();
    this.vhQueryAutoWeb.getSetups_byFields({type: {$in: ['food_image', 'category_image']}})
    .then((res:any) => {
      if(res.length) {
        this.setupFoodImg = res.find(e => e.type == 'food_image')
        this.setupCategoryImg = res.find(e => e.type == 'category_image')
      }
    })
  }

  ngOnDestroy(): void { }

  trackByFn(index: number, item: any) {
    return item._id; // hoặc sử dụng một thuộc tính duy nhất khác của item
  }

  languageChangedSubscription
  ngAfterViewInit(): void {
    // ko được để đoạn logic này trong ngAfterViewChecked vì sẽ gây lag
    this.languageChangedSubscription = this.vhEventMediator.configChanged.subscribe((message: any) => {
      if (message?.status === 'update-language') {
        this.keySearch=''; 
        this.foods=[]; 
        this.totalPages=0
      }
    });

    if (
      document.querySelector('#purchase-invoice-today') &&
      document.querySelector('.ant-table-thead') &&
      document.querySelector('.purchase-invoice-today-header')
    ) {
      this.tableHeight =
        document.querySelector('#purchase-invoice-today').clientHeight -
        document.querySelector('.ant-table-thead').clientHeight -
        document.querySelector('.purchase-invoice-today-header').clientHeight -
        100 +
        'px';
    }
    this.changeDetectorRef.detectChanges();
  }


  /**
   * hàm này render option danh mục có dấu - thể hiện cấp độ
   * @param category danh mục cần render
   * @returns 
   */
  renderOptionCategory(category) {
    if(category.step) {
      return Array(category.step).fill(0).map((_, i) => i).map(() => '- ').join('') + category['name_' + this.functionService.languageTempCode]
    }
    return category['name_' + this.functionService.languageTempCode]
  }

  /**
   * hàm này get danh sách category để chọn xem sp theo danh mục
   * @example this.getCategory()
   */
  getCategory() {
    this.loadingCategory = true;
    this.vhQueryAutoWeb
      .getCategorys_byFields(
        { id_main_sectors: { $all: ['food_drink'] } },
        {},
        {},
        0
      )
      .then((res: any) => {
        if (res.vcode === 0) {
          this.vhQueryAutoWeb
            .getCategorySteps_byIdCategoryArray(
              res.data.map((e) => {
                return e._id;
              })
            )
            .then(
              (response: any) => {
                if (response.vcode === 0) {
                  response.data.forEach((e) => {
                    this.categories = [
                      ...this.categories,
                      e
                    ]
                  });
                }
              },
              (error: any) => {
                console.log('error', error);
              }
            ).finally(() => this.loadingCategory = false);
        }
      });
  }

  /** 
   *  Hàm thực hiện lấy danh sách sản phẩm
   */
  getFoods() {
    this.loading = true;
    this.keySearch = '';
    let query:any  = { type: { $eq: 1 } }
    if(this.idCategory != 'all') {
      query = { 
        ...query,
        id_categorys: { $all: [this.idCategory] }, 
      }
    }
    this.vhQueryAutoWeb.getFoods_byFields(
      query,
      {},
      this.sort,
      this.limit,
      this.pageCurrent
    ).then((res: any) => {

      this.foods = this.proccessData(res.data);
      this.totalPages = res.totalpages;

      // new Event('resize') tạo ra một event có type = "resize".
      // Khi phát đi, tất cả các listener đang lắng nghe sự kiện resize của window sẽ được gọi.
      // height lúc chưa load sản phẩm sẽ khác height sau khi load sản phẩm xong nên cần trigger lại sự kiện resize để virtual scroll hoạt động chính xác
       window.dispatchEvent(new Event('resize'));
    })
    .catch((error) => console.error(error))
    .finally(() => this.loading = false)
  }


  // biến đổi dữ liệu get về để hiển thị
  proccessData(data) {
    // const validCategoryIds = this.categories.map(c => c._id);
    const dataClone = JSON.parse(JSON.stringify(data));
    const dataReturn = dataClone.map(food => {
        food.show_expand = false;
        if (food.units) {
          food.unitDefault = food.units.find((item) => item.default == true);
        }
        
        if (food.subs?.length) {
          food.subs.forEach((sub) => {
            sub.unitDefault = sub?.units?.find((e) => e.default);
          });
        }
        // Cập nhật chuỗi tên danh mục
        const name = Array.isArray(food.id_categorys)
          ? food.id_categorys
            .map((idCate) => {
              if (idCate === '') {
                return 'Trống';
              } else {
                return this.categories.find((find) => find._id === idCate)?.['name_' + this.functionService.selectedLanguageCode];
              }
            })
            .join(', ')
          : this.categories.find((find) => find._id === food.id_categorys)?.['name_' + this.functionService.selectedLanguageCode];
        food.category_name = name;

        // xử lý id_category đã bị xóa 
        // let id_categorys = food.id_categorys.filter(id => validCategoryIds.includes(id));
        // if (id_categorys.length !== food.id_categorys.length) {
        //   this.vhQueryAutoWeb.updateFood(food._id, { id_categorys: id_categorys })
        //     .then((res) => {
        //       console.log('Cập nhật id_categorys thành công', res);
        //       food.id_categorys = id_categorys;
        //     })
        //     .catch((err) => {
        //       console.error('Lỗi cập nhật id_categorys', err);
        //     });
        // }
        return food;
      });
      return dataReturn;
  } 
  
  /** Hàm thực hiển chọn đơn vị
   *
   * @param item sản phẩm
   * @param event đơn vị chọn
   * @returns đơn vị chọn và gán vào unitDefault
   */
  selecUnit(item, event) {
    return (item.unitDefault = item.units.find((item) => item == event));
  }


  /** Hàm thực hiện mở popup thêm sản phẩm
   *
   */
  addFoodFn(): void {
    if(!this.setupFoodImg) return;
    const dialogRef =  this.dialog.open(AddFoodComponent, {
      width: '60vw',
      height: '80vh',
      data: {
        categories: this.categories,
        setupFoodImg: this.setupFoodImg,
        setupCategoryImg: this.setupCategoryImg
      }

    });

    
    dialogRef.afterClosed().subscribe((result) => {
      this.functionService.languageTempCode = this.functionService.selectedLanguageCode;
      if (result && (this.idCategory == 'all' || result.id_categorys.includes(this.idCategory))) {
        this.foods = [this.proccessData([result])[0], ...this.foods];
      }
    });

  }

  /** Hàm thực hiện mở popup edit với id
   *
   * @param food: dữ liệu sản phẩm được truyền cho popup
   */
  dataEditFood: any;
  editFood(food): void {
    if(!this.setupFoodImg) return;
    const dialogRef = this.dialog.open(EditFoodComponent, {
      width: '60vw',
      height: '80vh',
      data: { 
        categories: this.categories,
        dataEditFood: food,
        setupFoodImg: this.setupFoodImg,
        setupCategoryImg: this.setupCategoryImg
      },
    });

    dialogRef.afterClosed().subscribe((result) => {
      this.functionService.languageTempCode = this.functionService.selectedLanguageCode;
      if(result) {
        this.foods = this.foods.map(p => p._id === result._id ? this.proccessData([result])[0] : p)
      }
    });
  }

  /** Hàm thực hiện xác nhận xóa sản phẩm và thực hiện xóa sản phẩm
   *
   * @param food
   */
  deleteFood(food) {
    this.vhComponent
      .alertConfirm('', this.languageService.translate('xoa_mon_an') + '?', food[this.nameField], this.languageService.translate('dong_y'), this.languageService.translate('thoat'))
      .then(
        (ok) => {
          if (ok == 'OK') {
            this.vhQueryAutoWeb.deleteFood(food._id).then(
              (res: any) => {
                if(res.vcode != 0 && res.vcode != 11) {
                  this.functionService.createMessage(
                    'error',
                    res.msg
                  );
                  return;
                }
                if (res.vcode === 0) {
                  this.foods = this.foods.filter((item) => item._id != food._id);
                  this.functionService.createMessage('success','xoa_thanh_cong');
                }
                if (res.vcode === 11) {
                  this.functionService.createMessage('error','phien_dang_nhap_da_het_han_vui_long_dang_nhap_lai');
                }
              },
              (error) => {
                console.error(error);
                this.functionService.createMessage('error','da_xay_ra_loi_trong_qua_trinh_xoa_du_lieu_vui_long_thu_lai');
              }
            );
          }
        },
        (error) => {
          console.error(error);
        }
      );
  }

  /** Hàm thực hiện cập nhật trạng thái sản phẩm
   *
   * @param item
   * @param objectUpdate
   */
  updateHiddenFood(item, objectUpdate) {
    this.vhQueryAutoWeb
      .updateFood(item._id, objectUpdate)
      .then((res: any) => {
        if (res.vcode === 0) {
          this.functionService.createMessage(
            'success',
            'cap_nhat_thanh_cong'
          );
        }
        if (res.vcode === 11) {
          this.functionService.createMessage('error','phien_dang_nhap_da_het_han_vui_long_dang_nhap_lai');
        }
      })
      .catch((err) => {
        console.error(err);
        this.functionService.createMessage('error','da_xay_ra_loi_trong_qua_trinh_cap_nhat_du_lieu_vui_long_thu_lai');
      });
  }


  onSearch(): void {
    this.idCategory = 'all';
    this.resetPagination();
    this.vhQueryAutoWeb.searchList_likeSearch('foods', this.nameField, this.keySearch, {}, {})
      .then((res: any) => {
        if (res.vcode === 0) {
          this.dataSearched = res.data;
          this.dataSearched.forEach((food) => {
            food.show_expand = false;
            if (food.units) {
              food.unitDefault = food.units.find((item) => item.default == true);
            }
            // Cập nhật chuỗi tên danh mục
            const name = Array.isArray(food.id_categorys)
              ? food.id_categorys
                .map((idCate) => {
                  if (idCate === '') {
                    return 'Trống';
                  } else {
                    return this.categories.find((find) => find._id === idCate)?.['name_' + this.functionService.selectedLanguageCode];
                  }
                })
                .join(', ')
              : this.categories.find((find) => find._id === food.id_categorys)?.['name_' + this.functionService.selectedLanguageCode];
    
            food.category_name = name;
          });
          this.handlePaginateLocal();
        }
      }, (error: any) => {
        console.error('error', error)
      })

  }

  /** Hàm thực hiện cập nhật trạng thái của sub food
   *
   * @param subfood
   * @param objectUpdate
   */
  updateHiddenSubFood(subfood, objectUpdate) {
    this.vhQueryAutoWeb.updateSubProduct(subfood._id, objectUpdate).then(
      (response: any) => {
        if (!response) {
          this.functionService.createMessage(
            'error',
            'Đã xãy ra lỗi trong quá trình cập nhật dữ liệu ! Vui lòng thử lại.'
          );
        }
      },
      (error) => {
        console.error(error);
        this.functionService.createMessage('error','da_xay_ra_loi_trong_qua_trinh_xoa_du_lieu_vui_long_thu_lai');
      }
    );
  }

  handlePaginateLocal() {
    let data_filter =  this.dataSearched
    switch (true) {
      case this.sort.hasOwnProperty(this.nameField): {
        data_filter = this.sort[this.nameField] == 1 ?  this.vhAlgorithm.sortStringbyASC(data_filter, this.nameField)  : this.vhAlgorithm.sortStringbyDESC(data_filter, this.nameField)
        break;
      }
    }
    let data_page = new Array(); //mảng dữ liệu phân theo page
    for (let i = 0; i < data_filter.length; i++) {
      if ((i >= this.limit * (this.pageCurrent - 1)) && (i < this.limit * this.pageCurrent)) data_page.push(data_filter[i]);
    }
    this.totalPages = Math.ceil(data_filter.length / this.limit); // tổng số page

    this.foods = data_page;
  }

    /**
 * hàm này reset pagination về 1
 */
  resetPagination() {
    this.pageCurrent = 1;
    this.pageGoto = 1;
    this.pageShowChoose = [0, 1, 2];
  }

  
  handleSort(field) {
    this.sortby[field] = !this.sortby[field];
    switch (field) {
      case this.nameField: {
        this.sort = { [this.nameField]: this.sortby[field] ? 1 : -1 };
        break;
      }
    }

    if(this.keySearch && this.dataSearched.length) {
      this.handlePaginateLocal();
    }else {
      this.getFoods();
    }
  }

  /** pageCurrent thay đổi */
  pageIndexChange(event) {
    this.pageCurrent = event;
    if (this.keySearch && this.dataSearched.length) {
      this.handlePaginateLocal();
    }
    else 
      this.getFoods();
  }

  /** limit thay đổi */
  limitChange(event) {
    this.resetPagination();
    this.limit = event;
    if(this.keySearch && this.dataSearched.length) {
      this.handlePaginateLocal();
    } else {
      this.getFoods()
    }
  }


}

...
export class NewsComponent implements OnInit {
  ...
  /** Lấy trường title theo ngôn ngữ đang được chọn */
  get titleField() {
    return 'title_' + this.functionService.selectedLanguageCode;
  }
  ...
  ngOnInit(): void {
    this.getCategory();
    this.vhQueryAutoWeb.getSetups_byFields({type: {$in: ['news_image', 'category_image']}})
    .then((res:any) => {
      if(res.length) {
        this.setupNewsImg = res.find(e => e.type == 'news_image')
  ...
  getCategory() {
    ...
    this.vhQueryAutoWeb
      .getCategorys_byFields({ id_main_sectors: { $all: ['news'] } }, {}, {}, 0)
      .then((res: any) => {
        if (res.vcode === 0) {
          this.vhQueryAutoWeb
  ...
  getNews() {
    ...
    this.vhQueryAutoWeb.getNewss_byFields(
      query,
      {},
      this.sort,
      this.limit,
      ...
      (res: any) => {
        ...
        if (res.vcode == 0) {
          ...
          this.news = this.proccessData(res.data);
      ...
      (error) => {
        ...
        this.functionService.createMessage(
          'error',
          this.languageService.translate('da_xay_ra_loi_trong_qua_trinh_lay_du_lieu')
        );
      }
    )
  ...
  proccessData(data) {
    ...
    const dataReturn = dataClone.map((product) => {
      ...
      //   this.vhQueryAutoWeb.updateNews(product._id, { id_categorys: id_categorys })
      //     .then((res) => {
  ...
  handleSort(field) {
    ...
    switch (field) {
      case this.titleField: {
        this.sort = { [this.titleField]: this.sortby[field] ? 1 : -1 };
        break;
      }
    }
  ...
  onSearchNews(): void { 
    ...
    this.vhQueryAutoWeb.searchList_likeSearch('newss', this.titleField, this.keySearch, {}, {})
      .then((res: any) => {
        if (res.vcode === 0) {
  ...
  handlePaginateLocal() {
    ...
    switch (true) {
      case this.sort.hasOwnProperty(this.titleField): {
        data_filter = this.sort[this.titleField] == 1 ?  this.vhAlgorithm.sortStringbyASC(data_filter, this.titleField) : this.vhAlgorithm.sortStringbyDESC(data_filter, this.titleField)
        break;
      }
    }
  ...
  deteleNews(news) {
    this.vhComponent
      .alertConfirm('', this.languageService.translate('xoa_bai_viet'), news[this.titleField], this.languageService.translate('dong_y'), this.languageService.translate('thoat'))
      .then((ok) => {
        if (ok == 'OK') {
          this.vhQueryAutoWeb.deleteNews(news._id).then(
            (res: any) => {
              if (res.vcode === 0) {
                ...
                this.functionService.createMessage(
                  'success',
                  this.languageService.translate('xoa_bai_viet_thanh_cong')
                );
              }
              if (res.vcode === 11) {
                this.functionService.createMessage(
  ...
  addNews(): void {
    if(!this.setupNewsImg) return;
    const dialogRef =  this.dialog.open(AddNewsComponent, {
      width: '60vw',
      height: '80vh',
      data: {
        categories: this.categories,
        setupNewsImg: this.setupNewsImg,
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if (result && (this.idCategory == 'all' || result.id_categorys.includes(this.idCategory))) {
        this.news = [this.proccessData([result])[0], ...this.news];
      }
    });
  }
  ...
  editNews(value): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      this.functionService.languageTempCode = this.functionService.selectedLanguageCode;
      if (result) {
        this.news = this.news.map(p => p._id === result._id ? this.proccessData([result])[0] : p)
      }
    });
    ...
    this.vhQueryAutoWeb.updateNews(news._id, objectUpdate).then(
      (res: any) => {
        if (res.vcode === 0) {
          this.functionService.createMessage(
            'success',
            this.languageService.translate('cap_nhat_bai_viet_thanh_cong')
          );
        }
        if (res.vcode === 11) {
          this.functionService.createMessage(
